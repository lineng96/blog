...
桶排序将 $[0,1)$ 区间划分为 $n$ 个相同大小的子区间，每个子区间称为一个“桶”。
1. **分发**：根据元素的值，将每个输入值放入到对应桶中。
2. **排序**：对每个桶中的元素进行排序（可以使用任何稳定的排序算法，通常使用插入排序）。
3. **合并**：按照桶的顺序，依次列出各个桶的元素。

步骤，假设数组 $A$，元素个数 $n$ ；
1. 创建一个大小为 $n$ 的辅助数组 $B$ ，其中每个位置 $B[i]$ 都是一个空链表（桶）；
2. 遍历数组 $A$ ，将每个元素放入对应的桶 $B[\lfloor n \cdot A[i] \rfloor]$ 中；
3. 对每个桶 $B[i]$ 中的元素进行排序（使用插入排序）；
4. 将每个桶 $B[i]$ 中的元素依次放回原数组 $A$ 中。
```cpp
...
```
时间复杂度：
* 平均情况：$O(n)$ ，当输入数据均匀分布在 $[0,1)$ 时，每个桶中的元素个数趋于常数。即使桶内排序使用了 $O(k^2)$ 算法，总时间仍是线性的。
* 最坏情况：$O(n^2)$ ，当输入数据集中在某个桶中时，桶内排序的时间复杂度会退化到 $O(n^2)$ 。
